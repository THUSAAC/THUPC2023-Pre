...
inline void build(){
	...
	memset(dpt,0,sizeof(dpt));memset(bj,0,sizeof(bj));memset(sl,0,sizeof(sl));
	tot = 0;
	int i,j,k,l,x,y;
...
inline int getd(int u){
	int as = 0;
	while(!bel[u]) ++as,u = f[u];
	while(bel[u] != u) ++as,u = f[u];
	while(u) as += sl[u],u = ff[u];
	return as;
}
int main(){
	srand(time(0));rd();
	...
	for(i = 1;i <= m;++i){
		...
		if(op == 1){
			...
			//print(getd(u) + getd(v) - 2 * getd(x) + 1);
			pc('\n');
		}
		if(i % M == 0) rebuild();
	}
