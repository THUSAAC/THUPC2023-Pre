...
struct out_entry{
	...
	void output(){
		...
		for(int i = 0;i < p1;++i){
			print(q1[i].first);pc(' ');print(q1[i].second.size());pc(' ');
			for(int j = 0;j < q1[i].second.size();++j) print(q1[i].second[j]),pc(' ');
			pc('\n');
		}
		for(int i = 0;i < p2;++i){
			print(q2[i].first);pc(' ');print(q2[i].second.size());pc(' ');
			for(int j = 0;j < q2[i].second.size();++j) print(q2[i].second[j]),pc(' ');
			pc('\n');
		}
		for(int i = 0;i < p3;++i){
			print(q3[i].second.size());pc(' ');
			for(int j = 0;j < q3[i].second.size();++j) print(q3[i].second[j]),pc(' ');
			pc('\n');
		}
		init();
	}
...
struct plane{
	...
	bool can_reach(const vec3 &x){//飞机能否飞行x距离（飞到p+x位置），能的话更新nxtp,nxtu,nxtd
		...
		/*
		if(x == (vec3){1,0,1} || x == (vec3){0,0,3})
		cerr<<id<<":"<<x.x<<" "<<x.y<<" "<<x.z<<":::"
			<<nxtd.x<<" "<<nxtd.y<<" "<<nxtd.z<<"* "
			<<nxtl.x<<" "<<nxtl.y<<" "<<nxtl.z<<"* "
			<<(d - nxtd).x<<" "<<(d - nxtd).y<<" "<<(d - nxtd).z<<"& "
			<<nxtu.x<<" "<<nxtu.y<<" "<<nxtu.z<<"* "
			<<l.get_angle(nxtl) / r<<" "
...
void find_target(plane &x){//无人机选定目标
	...
	for(int i = 1;i <= m;++i){
		...
		/*
		cerr<<x.id<<":"<<i<<":::"<<r.x<<" "<<r.y<<"   "
			<<x.in_radar_r(r)<<" "<<(x.p - y.p).dis2()<<" "
			<<r.dis_to_rectangle(x.lx,x.hy)<<endl;
		*/
		...
		for(int i = -vi;i <= vi;++i){
			for(int j = -vi;j <= vi;++j){
				for(int k = -vi;k <= vi;++k){
					...
					/*
					if(np == (vec3){1,0,1} || np == (vec3){0,0,3})
					cerr<<x.in_nxt_horizon(a[x.target])<<"()"<<endl;
					*/
	...
	for(int i = -vi;i <= vi;++i){
		for(int j = -vi;j <= vi;++j){
			for(int k = -vi;k <= vi;++k){
				...
				/*
				if(np == (vec3){-8,-5,-4} || np == (vec3){-8,-4,-5})
				cerr<<x.nxt_can_lock(a[x.target],tmp_angle)<<"&&&"<<endl;
				*/
				...
				if(x.target && x.nxt_can_lock(a[x.target],tmp_angle)){//有目标，且位移之后仍能锁定
					...
					if(np == (vec3){-8,-5,-4} || np == (vec3){-8,-4,-5})
					cerr<<ds<<" "<<min_tar_dis<<"***&&&"
						<<tmp_angle<<" "<<min_lock_angle<<"()"
						<<(ds - min_tar_dis)<<" "<<(tmp_angle - min_lock_angle)<<endl;
					*/
...
void missile_fly(missile &x){//导弹飞行
	...
	for(int i = 1;i <= m;++i){
		...
		//cerr<<x.id<<" "<<y.id<<"#"<<x.nxtp.x<<" "<<x.nxtp.y<<" "<<x.nxtp.z<<"***"<<y.p.x<<" "<<y.p.y<<" "<<y.p.z<<endl;
...
void debug_out(int nw){
	FILE *f;
	if(nw == 0) f = fopen("log2.txt","w");
	else f = fopen("log2.txt","a+");
	fprintf(f,"T = %d\n",nw);
	for(int i = 1;i <= m;++i){
		plane &pl = a[i];
