...
struct out_entry
{
	...
	{
		for (int i = 0; i < p1; ++i)
			sort(q1[i].se.begin(), q1[i].se.end());
		sort(q1.begin(), q1.end());
		for (int i = 0; i < p2; ++i)
			sort(q2[i].se.begin(), q2[i].se.end());
		sort(q2.begin(), q2.end());
		for (int i = 0; i < p3; ++i)
		{
			sort(q3[i].se.begin(), q3[i].se.end());
			q3[i].first = q3[i].se[0];
		}
...
{ // 无人机选定目标
	...
	{
		plane &y = a[i];
		if (y.status != ALIVE || y.team == x.team || !x.in_horizon(y))
			continue;
...
void get_plane_fly_info(plane &x)
{ // 求出无人机的飞行策略
	if (x.status != ALIVE)
		return;
	if (x.target)
		...
		for (int i = -vi; i <= vi; ++i)
		{
			for (int j = -vi; j <= vi; ++j)
			{
				for (int k = -vi; k <= vi; ++k)
				{
					if (!i && !j && !k)
						continue; // 不能原地不动
					if (i * i + j * j + k * k > x.vm * x.vm)
						continue;
					vec3 np = {(ldb)i, (ldb)j, (ldb)k};
					if (!x.can_reach(np))
						continue; // 必须要合法到达
					if (x.in_nxt_horizon(a[x.target]))
					{												// 目标在视野内
						ldb ds = (x.p + np - a[x.target].p).dis2(); // 到目标的距离
						if (ds - EPS > min_tar_dis)
							continue;
						...
						if (x.in_radar_r(r))
							...
							tmp2 = r.dis_to_rectangle(x.lx, x.hy);				   // 不在雷达范围内
						if ((ds < min_tar_dis - EPS)							   // 优先：距离最小
							|| (chkeq(ds, min_tar_dis) && tmp1 < min_len_rq - EPS) // 其次：在雷达范围内且距视野中心尽可能近
							|| (chkeq(ds, min_tar_dis) && min_len_rq == INF && tmp2 < min_dis_radar - EPS))
						{ // 再次：不在雷达范围内，距雷达范围尽可能近
							min_tar_dis = ds;
							min_len_rq = tmp1;
							min_dis_radar = tmp2;
							nxtp = x.nxtp;
...
{ // 所有无人机选定目标，并确定当前时刻内的飞行策略
	for (int i = 1; i <= m; ++i)
		find_target(a[i]);
	for (int i = 1; i <= m; ++i)
		get_plane_fly_info(a[i]);
}
/**************************STAGE 1 END************************************/

/**************************STAGE 2 BEGIN**********************************/
void launch_missile(plane &x)
{ // 发射导弹
	if (x.status != ALIVE || !x.tar_in_radar)
		return;
	if (b[x.id].status != DEAD)
		return;
...
bool chk_missile_explode(const vec3 &st, const vec3 &ed, const vec3 &tar, ldb dp)
{
	...
	if (st == ed)
		return (tar - st).dis2() <= dp * dp;
	segment seg = {st, ed};
	return seg.get_min_dis(tar) <= dp;
}
void get_missile_fly_info(missile &x)
{ // 求出导弹的飞行策略
	if (x.status == DEAD)
		return;
	int vi = floor(x.vm);
	...
	for (int i = -vi; i <= vi; ++i)
	{
		for (int j = -vi; j <= vi; ++j)
		{
			for (int k = -vi; k <= vi; ++k)
			{
				...
				if (x.target && x.nxt_can_lock(a[x.target], tmp_angle))
				{												   // 有目标，且位移之后仍能锁定
					ldb ds = (x.p + np - a[x.target].nxtp).dis2(); // 到目标即将飞到的位置的距离
					if ((ds < min_tar_dis - EPS)				   // 优先：距离最小
						|| (chkeq(ds, min_tar_dis) && tmp_angle < min_lock_angle - EPS))
					{ // 其次：锁定角最小
						min_tar_dis = ds;
...
{ // 导弹飞行
	...
	for (int i = 1; i <= m; ++i)
	{
		...
		if (x.nxtp == y.p // 导弹直接撞到飞机上，无论是否激活都摧毁
			|| (x.status == ACTIVE && chk_missile_explode(x.p, x.nxtp, y.p, x.dp)))
		{ // 判断激活的导弹x能否摧毁飞机y
			y.status = HANDLING;
			y.exploded.pb(x.id);
		}
...
/**************************STAGE 4 BEGIN**********************************/
void crash(plane &x, bool flag)
{ // 飞机坠毁
	if (x.status != HANDLING)
		return;
	if (!flag)
	{
		++out_en.p1;
	...
	x.status = DEAD;
	for (int i = 0; i < x.exploded.size(); ++i)
		b[x.exploded[i]].status = DEAD;
	for (int i = 1; i <= m; ++i)
	{ // 瞄准它的导弹立刻脱锁
		if (b[i].target == x.id)
			b[i].target = 0;
...
{ // 无人机飞行
	...
	{ // 检查飞机能否被别的导弹摧毁
		...
		if (y.status == DEAD)
			continue;
		if (x.nxtp == y.p // 导弹直接撞到飞机上，无论是否激活都摧毁
			|| (y.status == ACTIVE && chk_missile_explode(x.p, x.nxtp, y.p, y.dp)))
		{ // 判断激活的导弹y能否摧毁飞机x
			x.status = HANDLING;
			x.exploded.pb(i);
		}
...
{ // 导弹激活
	if (x.status != INACTIVE)
		return;
	if (a[x.id].status == DEAD || (x.p - a[x.id].p).dis2() > x.ds * x.ds)
		x.status = ACTIVE;
}
void stage9()
{ // 所有可激活的导弹被激活
