...
int pno;
int insert(int o, int l, int r, int p, int no){
	int ret = pt++;
	t[ret] = (st){pno,t[o].lc,t[o].rc};
	pno+=2;
	if(l==r){
		addedge(t[ret].no,no);
		return ret;
	...
	int mid=l+r>>1;
	if(p<=mid){
		t[ret].lc=insert(t[o].lc,l,mid,p,no);
	}
	else{
		t[ret].rc=insert(t[o].rc,mid+1,r,p,no);
	}
	addedge(t[ret].no,t[t[ret].lc].no);
	addedge(t[ret].no,t[t[ret].rc].no);
	return ret;
}
void insert(int o, int l, int r, int ll, int rr, int fr){
	if(o==0){
		return;
	}
	if(ll<=l&&rr>=r){
	...
	int mid=l+r>>1;
	if(ll<=mid){
		insert(t[o].lc,l,mid,ll,rr,fr);
	}
	if(rr>mid){
		insert(t[o].rc,mid+1,r,ll,rr,fr);
	}
}
void dfs(int o){
	low[o]=dfn[o]=++dfc;
